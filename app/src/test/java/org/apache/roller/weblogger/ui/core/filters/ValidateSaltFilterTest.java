...
public class ValidateSaltFilterTest {
    ...
    public void setUp() {
        ...
        filter = spy(new ValidateSaltFilter());
    }
    ...
    public void testDoFilterWithPostMethodAndValidSalt() throws Exception {
        ...
        // Override isIgnoredURL via getIgnoredUrlsProperty if needed, but here we test
        // non-ignored
    ...
    public void testDoFilterWithIgnoredURL() throws Exception {
        doReturn(rollerSession).when(filter).getRollerSession(request);
        doReturn(saltCache).when(filter).getSaltCache();
        // Return ignored URL property
        doReturn("https://example.com/app/ignoredurl?param1=value1&m2=value2").when(filter).getIgnoredUrlsProperty();
