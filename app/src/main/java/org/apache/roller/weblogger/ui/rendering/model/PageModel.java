...
public class PageModel implements Model {
    ...
    public void init(Map<String, Object> initData) throws WebloggerException {
        ...
        // CSRF salt (set by LoadSaltFilter, passed from PageServlet)
        this.salt = (String) initData.get("salt");

        // Context path for building correct AJAX URLs in templates
        String cp = (String) initData.get("contextPath");
        this.contextPath = (cp != null) ? cp : "";
    ...
    /**
     * Returns the CSRF salt token for use in AJAX POSTs from Velocity templates.
     */
    ...
    /**
     * Returns the servlet context path (e.g. "/roller") for building correct
     * AJAX URLs in Velocity templates regardless of deployment context.
     */
    ...
    /**
     * Returns true if the current weblog has been starred by the authenticated user.
     */
    public boolean isWeblogStarred() {
        try {
            if (pageRequest.getUser() == null) {
                return false;
            }
    ...
    /**
     * Returns true if the given entry has been starred by the authenticated user.
     * Accepts a raw {@link WeblogEntry} pojo.
     */
    public boolean isEntryStarred(WeblogEntry entry) {
        try {
            if (pageRequest.getUser() == null) {
                return false;
            }
    ...
    /**
     * Returns true if the given entry (as a {@link WeblogEntryWrapper}) has been
     * starred by the authenticated user. Velocity templates pass wrappers, so
     * this overload unwraps the pojo and delegates to
     * {@link #isEntryStarred(WeblogEntry)}.
     */
    public boolean isEntryStarred(WeblogEntryWrapper entryWrapper) {
        ...
        try {
            ...
            // Retrieve the real WeblogEntry by its id so we can call the manager.
            WeblogEntry entry = WebloggerFactory.getWeblogger()
                    .getWeblogEntryManager().getWeblogEntry(entryWrapper.getId());
            if (entry == null) {
                return false;
            }
            return WebloggerFactory.getWeblogger().getWeblogEntryManager()
