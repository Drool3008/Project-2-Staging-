...
public class JPAWeblogEntryManagerImpl implements WeblogEntryManager {
    ...
    public void saveComment(WeblogEntryComment comment) throws WebloggerException {
        ...
        // update weblog last modified date.  date updated by saveWebsite()
        roller.getWeblogManager().saveWeblog((Weblog) comment.getWeblogEntry().getWebsite());
    }
    ...
    public void saveWeblogEntry(WeblogEntry entry) throws WebloggerException {
        ...
        if (entry.isPublished()) {
            ...
            if (entry.getRefreshAggregates()) {
                ...
                for (WeblogEntryTag tag : entry.getTags()) {
                    updateTagCount(tag.getName(), (Weblog) entry.getWebsite(), 1);
                }
            } else {
        ...
        // update weblog last modified date.  date updated by saveWebsite()
        if(entry.isPublished()) {
            roller.getWeblogManager().saveWeblog((Weblog) entry.getWebsite());
        }
    ...
    public void removeWeblogEntry(WeblogEntry entry) throws WebloggerException {
        Weblog weblog = (Weblog) entry.getWebsite();
    ...
    private List<WeblogEntry> getNextPrevEntries(WeblogEntry current, String catName,
        ...
        if (catName != null) {
            category = getWeblogCategoryByName((Weblog) current.getWebsite(), catName);
            if (category != null) {
    ...
    private void removeWeblogEntryTag(WeblogEntryTag tag) throws WebloggerException {
        if (tag.getWeblogEntry().isPublished()) {
            updateTagCount(tag.getName(), (Weblog) tag.getWeblogEntry().getWebsite(), -1);
        }
    ...
    @Override
    public boolean isEntryStarredByUser(User user, WeblogEntry entry) throws WebloggerException {
        TypedQuery<Long> q = strategy.getNamedQuery(
                "WeblogEntry.countStarredByUser", Long.class);
    ...
    public List<Object[]> getTrendingEntries(int limit) throws WebloggerException {
        // Single aggregate GROUP BY query — counts stars per entry without
        // iterating over individual articles in application code.
        try {
            jakarta.persistence.EntityManager em = strategy.getEntityManager(true);
            Query q = em.createNamedQuery("WeblogEntry.getTopStarredEntries");
            q.setHint("jakarta.persistence.cache.retrieveMode",
                      jakarta.persistence.CacheRetrieveMode.BYPASS);
            ...
            throw new WebloggerException("Error fetching trending entries", e);
        }
    }
