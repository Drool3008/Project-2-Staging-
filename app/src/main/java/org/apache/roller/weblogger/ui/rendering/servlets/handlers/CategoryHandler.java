...
/**
 * Handler for category-based archive page requests.
 * 
 * Responsible for:
 * - Matching requests that have a category specified
 * - Validating that the category exists in the weblog
 * - Loading the category archive template (uses default template)
 * - Building the rendering model with category data
 * - Rendering and returning the content
 */
public class CategoryHandler implements PageRequestHandler {
    ...
    public boolean matches(WeblogPageRequest request) {
        ...
        return "category".equals(request.getContext()) &&
                request.getWeblogCategoryName() != null;
    }
    ...
    public CachedContent handle(WeblogPageRequest request, HttpServletRequest servletRequest,
        ...
        log.debug("CategoryHandler: handling category archive request");

        String categoryName = request.getWeblogCategoryName();

        // Load and validate category exists
        WeblogCategory category = request.getWeblogCategory();

        if (category == null) {
            throw new InvalidRequestException("Category not found: " + categoryName);
        }

        log.debug("Category validation passed");

        // Load the page template (use default template for category view)
        ...
        pageInitData.put("category", category);
        ...
        try {
            ...
            log.error("Error loading model objects for category page", ex);
            throw ex;
        }
